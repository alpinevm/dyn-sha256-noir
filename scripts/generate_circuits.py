...
class MemProfiler:
    def __init__(self, sample_rate=1.0):
        self.sample_rate = sample_rate  # time in seconds between checks
        self.max_mem = 0
        self.init_mem = 0
        self.process = None
    ...
    def track_memory_usage(self):
        while True:
            ...
            if (current_mem - self.init_mem) > self.max_mem:
                self.max_mem = current_mem - self.init_mem
                print(f"New max memory usage: {self.max_mem} MB")
            time.sleep(self.sample_rate)

    def start(self):
        # Start the memory tracking process
        self.init_mem = psutil.virtual_memory().used / (1024 * 1024)
        self.process = multiprocessing.Process(target=self.track_memory_usage)
        self.process.start()

    def stop(self):
        # Stop the memory tracking process
...
profiler = MemProfiler(sample_rate=0.5)  # sample rate set to 0.5 seconds
...
async def profile_memory_write_vk():
    constants = json.load(open("scripts/constants.json", "r"))
    BB_BINARY = constants["BB_BINARY"]
    MAX_BYTES = 40000
    CHUNK_SIZE = 2000
    MAX_CHUNKS = int(MAX_BYTES / CHUNK_SIZE)

    mem_data = {}
    temp_dir = initiate_nargo_dir()
    for i in range(20, MAX_CHUNKS):

        def log_mem(ram_consumed: Any):
            mem_data[i * CHUNK_SIZE] = ram_consumed

        print("Starting", i * CHUNK_SIZE)
        await create_new_circuit(
            BB_BINARY, temp_dir.name, i * CHUNK_SIZE, False, log_mem, False
        )
        await create_demo_proof(BB_BINARY, temp_dir.name, i * CHUNK_SIZE)
        print("Finished", i * CHUNK_SIZE)
...
if __name__ == "__main__":
    ...
    # asyncio.run(profile_memory_write_vk())
